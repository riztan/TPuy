...
oTPuy:oIvan := tscript():New( "ivan" )
? ValType( oTpuy:oIvan ), "  ---   ",oTpuy:oIvan:ClassName()
return .t.

            PRG := memoread(cFile)
            HRB := hb_compileFromBuf( PRG, "harbour", "-n2", "-w0", "-es2", "-q0", ; 
                                    s_aIncDir, "-I" + FNameDirGet( cFile )  )
QOUT( "VALTYPE ",VALTYPE(HRB) )
QOUT( HRB )
QOUT()
            //HRB := hb_hrbLoad( cFile )
//hb_hrbRun(HRB)
            HRB_HAND := hb_hrbLoad( HRB )
            func_handle := ( hb_hrbGetFunSym( HRB_HAND, "SCRIPTMAIN" ) )
            EVAL( func_handle )
...
//MsgInfo( cFile ) 
            ...
            if !oTpuy:lMainRun
               ...
               PRG := memoread(cFile)
               cFile := hb_compileFromBuf( PRG, "-n2", "-w0", "-es2", "-q0",;
                                       s_aIncDir, "-I" + FNameDirGet( cFile )  )
            endif
            If HB_ISNIL(cFile)
               If file(cLogFile)
//                  MsgStop(memoread(cLogFile),"Error")
         ...
         OTHERWISE
            ...
            //if oTpuy:lMainRun
            //  hb_hrbLoad( cFile )
            //  func_handle := ( hb_hrbGetFunSym(cFile, "SCRIPTMAIN" ) )
              //Eval( func_handle )
            //else
              hb_hrbRun( cFile, ... )
            //endif
